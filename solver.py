...
class Solver(object):
    def __init__(self, config, svhn_loader, mnist_loader):
        ...
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # Add device attribute
        self.g12 = None
        self.g21 = None
        self.d1 = None
        self.d2 = None
    ...
    def build_model(self):
        ...
        self.g12 = G12(conv_dim=self.g_conv_dim).to(self.device)
        self.g21 = G21(conv_dim=self.g_conv_dim).to(self.device)
        self.d1 = D1(conv_dim=self.d_conv_dim, use_labels=self.use_labels).to(self.device)
        self.d2 = D2(conv_dim=self.d_conv_dim, use_labels=self.use_labels).to(self.device)
    ...
    def save_images(self, images, size, image_path):
        images = ((images + 1) * 127.5).astype(np.uint8)  # Convert to uint8
    ...
    def merge_images(self, sources, targets, k=10):
        ...
        return merged.transpose(1, 2, 0).astype(np.uint8)
    ...
    def train(self):
        iter_per_epoch = min(len(self.svhn_loader), len(self.mnist_loader))
        
        # Fixed MNIST and SVHN for sampling
        fixed_svhn = self.to_var(next(iter(self.svhn_loader))[0])
        fixed_mnist = self.to_var(next(iter(self.mnist_loader))[0])
        ...
        for step in range(self.train_iters + 1):
            ...
            if (step + 1) % iter_per_epoch == 0:
                ...
                if 'svhn_iter' not in locals():
                    svhn_iter = iter(self.svhn_loader)
                if 'mnist_iter' not in locals():
                    mnist_iter = iter(self.mnist_loader)
            ...
            if (step + 1) % self.sample_step == 0:
                ...
                imageio.imwrite(path, merged)  # Use imageio to save the image
                print('Saved %s' % path)
